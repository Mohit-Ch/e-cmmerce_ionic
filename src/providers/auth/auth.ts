...
export class AuthProvider {
  ...
  public setorderincart(itemId: string, itemeditionId:string, quantity:string) {
              ...
              if(cartorder==null)
              {
                ...
                // find data exist in the car or not   
                  ...
                  if(exist!=null)
                  {
                    if(quantity=="0")
                    {
                      const index: number =cartList.indexOf(x=>x.itemId==itemId && x.itemeditionId==itemeditionId);
                      if (index !== -1) {
